			...
			<div class="col-xs-12 col-md-9 write-wrap">
				...
				<form>
					...
					<button type="button" class="btn btn-primary"
						onclick="location.href='modify?qno=${vo.qno}&qid=${vo.qid}'">변경</button>
						...
						<div class="reply-group">
							<div class="reply-input">
								<input type="text" class="form-control" placeholder="이름"
									id="rid" value="${userVO.userId}" readonly> 
							</div>
	...
	$(document).ready(function() {
			...
			$.getJSON(
				"replylist/"+qno+"/"+page,function(map){
					...
					for(var i=0; i<data.length; i++){
						...
						stradd+="<a href='"+data[i].rno+" "+data[i].rid+"' class='right replyModify'><span class='glyphicon glyphicon-pencil'></span>수정</a>"
						stradd+="<a href='"+data[i].rno+" "+data[i].rid+"' class='right replyDelete'><span class='glyphicon glyphicon-remove'></span>삭제</a>"
						stradd+="</div>";
		...
		*/
		...
		$("#replyList").on("click","a",function(){
			...
			var rno=$(this).attr("href").split(" ")[0]
			var rid=$(this).attr("href").split(" ")[1]
		...
		$("#modalModBtn").click(function () {
			...
			$.ajax({
				type:"post",
				url:"replyupdate",
				data:JSON.stringify({"rno":rno,"rcontent":rcontent}),
				contentType:"application/json; charset=utf-8",
				success:function(data){
					if(data==1){
						alert('업데이트완료')
